...
int main() {
    ...
    if (file.is_open()) {
        ...
        if (checkInfixToPostfix(infixExpr)) {
            std::cout << "Данная инфиксная форма может быть переведа в постфиксную" << std::endl;
        }
        else {
            std::cout << "Данная инфиксная форма НЕ может быть переведа в постфиксную" << std::endl;
        }
    }
    else {
